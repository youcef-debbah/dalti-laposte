 ...
 * to the execution hooks ({@link #doFromBackground(Object)} and
 * {@link #closeFromBackground(Object)}). The context will get nulled
 ...
 */
@AnyThread
public abstract class UnDatabaseJob<C> extends AbstractJob {
     ...
     * {@linkplain AppWorker#DATABASE database worker}
     * and not {@linkplain #isReusable() reusable}.
     *
     * @param context the context instance that will be available as a parameter
     *                to the execution hooks ({@link #doFromBackground(Object)}
     *                and {@link #closeFromBackground(Object)})
     */
    protected UnDatabaseJob(@NonNull final C context) {
        super(AppWorker.DATABASE, false);
     ...
     */
    protected UnDatabaseJob(@NonNull final C context, final boolean reusable) {
        super(AppWorker.DATABASE, reusable);
        this.contextHolder = new AtomicReference<>(Check.nonNull(context));
    }
     ...
     *                to the execution hooks ({@link #doFromBackground(Object)}
     *                and {@link #closeFromBackground(Object)})
     * @throws RuntimeException is the worker is backed by the main thread
     */
    protected UnDatabaseJob(@NonNull final AppWorker worker,
                            @NonNull final C context) {
        super(Check.nonNull(worker), false);
        Check.not(worker.isBackedByMainThread());
        this.contextHolder = new AtomicReference<>(Check.nonNull(context));
    }
     ...
     */
    @MainThread
    protected final void doFromMain(@NonNull final C context) {
     ...
     */
    @MainThread
    protected final void closeFromMain(@NonNull final C context) {
    }
